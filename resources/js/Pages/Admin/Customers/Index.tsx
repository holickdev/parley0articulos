...
export default function Index({ customers }: { customers: Customer[] }) {
    ...
    const [sortBy, setSortBy] = useState<'name' | 'identification'>('name');
    ...
    const toggleSort = (column: 'name' | 'identification') => {
        if (sortBy === column) {
                ...
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                        ...
                        <PrimaryButton onClick={openCreateModal}>
                            Registrar Cliente <span className="ml-2 text-lg">+</span>
                        </PrimaryButton>
                    </div>
                    ...
                    <div className="overflow-hidden bg-white shadow-sm border border-parley-gold/50">
                            ...
                            <table className="min-w-full divide-y divide-parley-gold/20">
                                <thead className="bg-parley-cream">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider text-parley-brown/60">
                                            Teléfono
                                        </th>
                                ...
                                <tbody className="divide-y divide-parley-gold/20 bg-white">
                                    {paginatedCustomers.map((customer) => (
                                        <tr key={customer.id} className="hover:bg-parley-cream/30 transition-colors">
                                            <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-parley-brown">
                                                {customer.identification}
                                            </td>
                                            <td className="whitespace-nowrap px-6 py-4 text-sm font-bold text-parley-brown">
                                                {customer.name}
                                            </td>
                                            <td className="whitespace-nowrap px-6 py-4 text-sm text-parley-brown/70">
                                                {customer.phone}
                                            </td>
                                    ...
                                    {paginatedCustomers.length === 0 && (
                                        <tr>
                                            <td colSpan={4} className="px-6 py-10 text-center text-parley-brown/40 italic">
                        ...
                        <div className="bg-parley-cream p-4 border-t border-parley-gold/50 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div className="text-sm text-parley-brown/80 font-medium">
                                Mostrando <span className="text-parley-brown font-bold">{paginatedCustomers.length}</span> de <span className="text-parley-brown font-bold">{filteredCustomers.length}</span> registros
                            </div>
                            {totalPages > 1 && (
                                <div className="flex items-center gap-2">
                                    <button 
